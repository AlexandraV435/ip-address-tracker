<template>
  <header>
    ...
    <div class="wrapper">
      <input @keyup.enter="get_IP_name(`https://api.ipgeolocation.io/ipgeo?apiKey=f2bdffdfe7584be9b0e9ab024dcebbd9&ip=${ip_name}`)" v-model="ip_name" type="text" aria-label="Default" placeholder="Search for any IP address or domain">
      <button @click="get_IP_name(`https://api.ipgeolocation.io/ipgeo?apiKey=f2bdffdfe7584be9b0e9ab024dcebbd9&ip=${ip_name}`)">
          <div></div>
      </button>
    </div>
  </header>
    ...
    methods: {
      get_IP_name(ip) {
        fetch(ip).then(res => res.json()).then(data => {
          ...
          this.map.setView(new L.LatLng(data.latitude, data.longitude), 15);
